...
| Learning Objective | `{official_code}` or `{major.minor.patch}` (SemVer) | `5.1.1`, `9.2.3`, `1.0.0` |
...
**Format:** `{official_code}` — use the official numbering from the syllabus standard document (e.g. DSKP, CBSE syllabus). Fall back to **Semantic Versioning (SemVer)** format `{major.minor.patch}` when no official numbering exists.
...
# Fallback — SemVer format when the syllabus has no official LO codes
1.0.0
1.1.0
2.0.0
```
...
- **Major** — learning objective group or strand (increment for each new group)
- **Minor** — sub-objective within the group (increment for each sub-objective)
- **Patch** — revision or granular variant (start at `0`, increment for refinements)
