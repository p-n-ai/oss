...
When contributing new curriculum content, all IDs and folder names must follow these conventions:
- IDs use the official MOE language of the target country
- Every YAML file includes both `name` (MOE language) and `name_en` (English)
- Every learning objective includes both `text` and `text_en`
...
Misformatted IDs are caught by the validator and the PR will be blocked.
